...
// gets the cyan wash rather than just the matched substring. Add span
      ...
      <div className="space-y-2">
        {citations.map((citation) => {
          ...
          const meta = [citation.regulation, citation.jurisdiction, citation.section, citation.clause, citation.page ? `p. ${citation.page}` : null]
            ...
            .join(' · ')
          return (
            <div key={citation.citation_id} className="bg-panel-raised border border-seam rounded-sm text-xs overflow-hidden">
              <button
                type="button"
                onClick={() => setOpenId(isOpen ? null : citation.citation_id)}
                className="w-full flex gap-2 p-2.5 text-left hover:bg-panel transition-colors"
              >
                <FileText className="w-3.5 h-3.5 text-signal flex-shrink-0 mt-0.5" />
              ...
              >
                <div className="px-2.5 pb-2.5">
                  <p className="text-ink leading-relaxed bg-signal/[0.15] border border-signal/20 rounded-sm p-2.5 whitespace-pre-wrap">
