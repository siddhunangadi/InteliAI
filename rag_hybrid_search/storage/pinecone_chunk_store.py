...
_LEGAL_FILTER_KEYS = {
    ...
    "clause", "document_type", "risk_category", "is_current", "superseded_by",
}
...
class PineconeChunkStore(ChunkStore):
    ...
    def put(self, chunk: Chunk, source_path: str | None = None) -> None:
        ...
        self._client.index.upsert(vectors=[{
            "id": chunk.chunk_id,
    ...
    def put_many(self, chunks: list[Chunk], source_path: str | None = None) -> None:
        ...
        with ThreadPoolExecutor(max_workers=len(batches) or 1) as executor:
            futures = [executor.submit(self._client.index.upsert, vectors=batch) for batch in batches]
    ...
    def get(self, chunk_id: str) -> Chunk | None:
        result = self._client.index.fetch(ids=[chunk_id])
