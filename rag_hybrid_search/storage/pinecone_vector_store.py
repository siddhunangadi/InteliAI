...
class PineconeVectorStore(VectorStore):
    ...
    def upsert(self, chunk_id: str, embedding_record: EmbeddingRecord) -> None:
        ...
        self._client.index.update(id=chunk_id, values=embedding_record.embedding)
    ...
    def query(self, embedding: list[float], k: int) -> list[tuple[str, float]]:
        result = self._client.index.query(vector=embedding, top_k=k, include_metadata=False)
        return [(m.id, m.score) for m in result.matches]

    def delete(self, chunk_ids: list[str]) -> None:
        self._client.index.delete(ids=chunk_ids)
