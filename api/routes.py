...
def _ingest_bytes(
    ...
    ``existing_pairs``/``rebuild_bm25`` let a batch caller share one dedup
    cache and defer the BM25 rebuild across many files -- see
    IngestionPipeline.ingest docstring.
    """
    try:
...
async def index_documents(
    ...
    """
    results = [
        _ingest_one(document, container)
...
async def diagnostics(
    ...
    # Build safe config summary (API keys scrubbed)
    settings_dict = container.settings.model_dump()
    for key in ["gemini_api_key", "nvidia_api_key", "pinecone_api_key"]:
        if key in settings_dict:
            settings_dict[key] = bool(settings_dict[key])  # True/False, not the actual key
