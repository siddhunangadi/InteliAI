...
Swap `MockProvider` for `NvidiaProvider` to generate against a real model:

```bash
export RAG_NVIDIA_API_KEY=your_api_key
```

```python
from rag_hybrid_search.providers.nvidia import NvidiaProvider

pipeline = RagPipeline(retriever=retriever, generation_provider=NvidiaProvider(api_key="..."))
```

`NvidiaProvider` (`rag_hybrid_search/providers/nvidia.py`) handles both generation and embeddings.
...
Provider selection is automatic based on which API keys are set in the environment (`RAG_NVIDIA_API_KEY`): with no key set, it falls back to `MockProvider` for generation and a deterministic `FakeEmbeddingProvider` for embeddings — useful for trying the API without any credentials, but `/answer` won't produce a grounded, real answer in that mode. `GET /health` reports which providers were actually selected. Production requires `RAG_NVIDIA_API_KEY` for both generation and embeddings.
